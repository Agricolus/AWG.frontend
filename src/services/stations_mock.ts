...
class StationsServicesMOCKED extends BaseRestService {
  ...
  constructor() {
    ...
    this.fakestations = [];
    for (let i = 0; i < 100; i++) {
      let coord = [42.0 + (Math.random() * i), 12.0 + (Math.random() * i)]
      let station = {
        id: "station-" + i,
        type: "station-type",
      ...
      this.fakestations.push(station);
    }
  }
  ...
  async getAllActiveStations(skip: number = 0, take: number = 20): Promise<dto.Paginated<dto.Device>> {
    ...
    //let megastations = [].concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS).concat(__MOKED_STATIONS_DATAS);
    // return __MOKED_STATIONS_DATAS;
    return {
      totalCount: this.fakestations.length,
      skip: skip,
      take: take,
      items: this.fakestations.slice(skip, skip + take)
    }
  }
  ...
  async getStation(stationId: string): Promise<dto.Device> {
    ...
    return (this.fakestations.find(d => d.id == stationId) || null) as dto.Device;
  }
